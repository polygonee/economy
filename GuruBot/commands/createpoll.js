async function getPollOptions(collector) {
...
const vote = async function(message, args, Discord, userCreatedPolls){
    if(userCreatedPolls.has(message.author.id)) return message.reply('You already have a poll going on.')
    ...
    if(reaction.emoji.name === '✅') {
        ...
        processPollResults(voteCollector, pollOptions, userVotes, pollTally);
...
module.exports = {
    name: 'createpoll',
    description: 'creates a poll with voting',
    execute(message, args, Discord, userCreatedPolls){
        vote(message, args, Discord, userCreatedPolls);
    }
}